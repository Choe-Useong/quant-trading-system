...
from kr_stock_live.trading.state import read_json, state_path_for_profile, write_json
from live_common.target_weights import _resolve_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Scheduler-safe Korean-stock/ETF live auto rebalance runner.")
    ...
    parser.add_argument("--run-dir", default="", help="Optional output dir. Defaults to kr_stock_live/.cache/runs/<profile>/auto.")
    parser.add_argument("--equity-krw", type=float, default=None, help="Optional equity override for plan generation.")
    parser.add_argument("--cash-krw", type=float, default=None, help="Optional cash override for plan generation.")
    parser.add_argument("--min-order-krw", type=float, default=10000.0)
    parser.add_argument(
        "--bootstrap-policy",
        choices=["empty-account", "always", "never"],
        default="empty-account",
        help="Initial state handling.",
    )
    parser.add_argument("--execute", action="store_true", help="Submit real KIS orders. Omit for dry-run.")
    parser.add_argument("--confirm-live", action="store_true", help="Required for KIS_ENV=live execution.")
...
def _run_dir(profile_json: Path, explicit: str) -> Path:
    ...
    return ROOT_DIR / "kr_stock_live" / ".cache" / "runs" / profile_json.stem / "auto"
...
def _write_auto_state(path: Path, payload: dict[str, Any]) -> None:
    payload.setdefault("type", "kr_stock_live_auto_state_v1")
...
def _summary_line(payload: dict[str, Any]) -> str:
    ...
    if payload_type == "kr_stock_live_rebalance_plan_v1":
        return (
            "plan "
            f"phase={payload.get('phase')} "
            f"allowed={payload.get('rebalance_allowed')}({payload.get('rebalance_reason')}) "
            f"orders={len(_nonzero_plan_orders(payload))} "
            f"buy={float(payload.get('planned_buy_notional_krw') or 0):.0f}KRW "
            f"sell={float(payload.get('planned_sell_notional_krw') or 0):.0f}KRW"
        )
    if payload_type == "kr_stock_live_execution_report_v1":
        return (
            "execution "
            f"phase={payload.get('phase')} "
    ...
    if payload_type == "kr_stock_live_fill_report_v1":
        return (
            "fills "
    ...
    if payload_type == "kr_stock_live_auto_report_v1":
...
def _plan_command(*, phase: str, profile_json: Path, state_json: Path, plan_path: Path, args: argparse.Namespace) -> list[str]:
    command = _py_command(
        "kr_stock_live/trading/rebalance_plan.py",
        "--profile-json",
        str(profile_json),
        "--state-json",
        str(state_json),
    ...
    _maybe_num_arg(command, "--equity-krw", args.equity_krw)
    _maybe_num_arg(command, "--cash-krw", args.cash_krw)
    return command


def _execute_command(*, plan_path: Path, state_json: Path, report_path: Path, args: argparse.Namespace) -> list[str]:
    command = _py_command(
        "kr_stock_live/trading/execute_plan.py",
        "--plan-json",
        str(plan_path),
        "--state-json",
        str(state_json),
...
def _fill_command(*, execution_path: Path, output_path: Path, args: argparse.Namespace) -> list[str]:
    return _py_command(
        "kr_stock_live/trading/check_fills.py",
        "--execution-json",
        str(execution_path),
        "--fill-filter",
        "01",
...
def _update_cache(profile_json: Path, *, skip: bool) -> None:
    if skip:
        return
    _run(_py_command("kr_stock_live/data/update_cache.py", "--profile-json", str(profile_json)))
...
def _write_report(run_dir: Path, payload: dict[str, Any]) -> None:
    payload.setdefault("type", "kr_stock_live_auto_report_v1")
...
def _pending_execution_path(auto_state: dict[str, Any], phase: str) -> Path | None:
    value = auto_state.get(f"{phase}_execution")
    if not value:
        return None
    return _resolve_path(str(value))


def _mark_done(*, plan_path: Path, state_json: Path) -> None:
    _run(
        _py_command(
            "kr_stock_live/trading/mark_executed.py",
            "--plan-json",
            str(plan_path),
            "--state-json",
            str(state_json),
...
def _handle_pending(
    *,
    ...
    if not fills.get("all_submitted_orders_fully_filled") or fills.get("weak_match_used"):
        ...
        _write_auto_state(auto_state_path, auto_state)
        _write_report(run_dir, {"action": "wait", "phase": phase, "reason": auto_state["reason"], "fills": _rel(fills_path)})
        return "stop"
    ...
    if str(auto_state.get(f"{phase}_submit_status") or "") == "partial_submitted":
        ...
        _write_auto_state(auto_state_path, auto_state)
        _write_report(run_dir, {"action": "replan", "phase": phase, "reason": auto_state["reason"], "fills": _rel(fills_path)})
        return "stop"
    ...
    if phase == "buy":
        ...
        _mark_done(plan_path=buy_plan, state_json=state_json)
...
def _run_new_phase(
    *,
    ...
    if not submitted:
        ...
        _write_auto_state(auto_state_path, auto_state)
        _write_report(run_dir, {"action": "blocked", "phase": phase, "reason": auto_state["reason"], "execution": _rel(execution_path)})
        return True
    ...
    _write_report(
        run_dir,
        {"action": "submitted" if args.execute else "dry_run", "phase": phase, "reason": auto_state["reason"], "execution": _rel(execution_path)},
    )
    return True
...
def main() -> int:
    ...
    if active_profile is not None:
        auto_state_update["profile_reference_json"] = _rel(profile_reference_json)
    auto_state.update(auto_state_update)
    ...
    if args.execute and phase == "sell_submitted":
        ...
        return 0
    if args.execute and phase == "buy_submitted":
        _handle_pending_with_retries(
            phase="buy",
            auto_state=auto_state,
            auto_state_path=auto_state_path,
